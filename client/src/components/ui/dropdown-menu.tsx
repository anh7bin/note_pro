...
>(({ className, inset, children, ...props }, ref) => (
    <DropdownMenuPrimitive.SubTrigger
        ref={ref}
        className={cn(
            'flex min-h-8 cursor-pointer select-none items-center gap-2 rounded-lg px-2.5 py-2 text-sm outline-none transition-colors duration-150',
...
>(({ className, sideOffset = 6, ...props }, ref) => (
    <DropdownMenuPrimitive.SubContent
        ...
        className={cn(
            'z-[70] min-w-36 overflow-hidden rounded-xl border border-border/70 bg-popover p-1.5 text-popover-foreground shadow-lg',
...
>(({ className, sideOffset = 6, ...props }, ref) => (
    <DropdownMenuPrimitive.Portal>
        <DropdownMenuPrimitive.Content
            ...
            sideOffset={sideOffset}
            className={cn(
                'z-[70] max-h-[var(--radix-dropdown-menu-content-available-height)] min-w-36 max-w-[calc(100vw-1rem)] overflow-y-auto overflow-x-hidden rounded-xl border border-border/70 bg-popover p-1.5 text-popover-foreground shadow-lg',
                'data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:duration-100 data-[state=open]:duration-150',
                'data-[side=bottom]:slide-in-from-top-1 data-[side=left]:slide-in-from-right-1 data-[side=right]:slide-in-from-left-1 data-[side=top]:slide-in-from-bottom-1 origin-[--radix-dropdown-menu-content-transform-origin]',
                className
            )}
...
>(({ className, inset, ...props }, ref) => (
    <DropdownMenuPrimitive.Item
        ref={ref}
        className={cn(
            'relative flex min-h-9 cursor-pointer select-none items-center gap-2 rounded-lg px-2.5 py-2 text-sm outline-none transition-colors duration-150',
...
>(({ className, children, ...props }, ref) => (
    <DropdownMenuPrimitive.RadioItem
        ref={ref}
        className={cn(
            'relative flex min-h-9 cursor-pointer select-none items-center gap-2 rounded-lg py-2 pl-2.5 pr-9 text-sm outline-none transition-colors duration-150',
            'focus:bg-accent focus:text-accent-foreground data-[state=checked]:bg-accent/50 data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
            className
        )}
        {...props}>
