...
DECLARE
  ...
  notification_permission_type TEXT;
BEGIN
  IF TG_OP = 'UPDATE'
  ...
  permission_label := CASE
    ...
    ELSE 'viewer'
  END;
  notification_permission_type := NEW.permission_type;
  ...
  ELSIF TG_OP = 'UPDATE'
        ...
        AND OLD.permission_type = 'write'
        AND NEW.status = 'approved'
        AND NEW.permission_type = 'read' THEN
    recipient_id := NEW.requester_id;
    notification_type := 'access_denied';
    notification_title := document_title;
    notification_message := 'Your request to edit this document was declined. You can still view it';
    notification_permission_type := OLD.permission_type;
  ...
  INSERT INTO public.notifications (
    ...
    jsonb_build_object(
      ...
      'permission_type', notification_permission_type,
